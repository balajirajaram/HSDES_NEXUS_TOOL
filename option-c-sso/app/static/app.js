...
async function loadAccount() {
  try {
    ...
    const name = signedIn ? data.user.name || data.user.email : "not signed in";
    document.getElementById("account-name").textContent = name;
    document.getElementById("login-btn").classList.toggle("hidden", signedIn);
    document.getElementById("logout-btn").classList.toggle("hidden", !signedIn);
    document.getElementById("signin-notice").classList.toggle("hidden", signedIn);
    setBadge("badge-auth", "auth: " + (signedIn ? "signed in" : "signed out"), signedIn);
  } catch (e) {
    /* ignore */
  }
}
...
document.getElementById("analyse-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (r.status === 401) {
      report.innerHTML = `<p class="error">Please <a href="/login">sign in</a> first.</p>`;
      return;
    }
    const data = await r.json();
    if (!r.ok) {
...
async function loadKB() {
  ...
  if (r.status === 401) {
    list.innerHTML = `<p>Please <a href="/login">sign in</a> to view the KB.</p>`;
    return;
  }
  const data = await r.json();
