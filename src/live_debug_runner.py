...
LIVE_DEBUG_TEMPLATE_PATH = SCRIPT_DIR / "live_debug_report_template.html"
...
def generate_html_report(session_data: dict, output_path: Path,
    ...
    if not LIVE_DEBUG_TEMPLATE_PATH.exists():
        raise FileNotFoundError(
            ...
            "Ensure live_debug_report_template.html is present in the hsd-triage folder."
        )
...
def generate_markdown_report(session_data: dict, output_path: Path,
    ...
    lines = [
        f"# Live Debug Session — HSD {session.get('hsd_id', '?')}",
        "",
    ...
    lines += [
        "---",
        "",
        f"*Generated by HSD Live Debug Agent — {_now_iso()}*",
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hsd-id", required=True, help="HSD ID to debug")
    p.add_argument(
