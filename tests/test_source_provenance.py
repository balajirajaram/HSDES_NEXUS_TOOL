...
class TestSourceProvenance(unittest.TestCase):
    def test_inventory_marks_internal_import_as_non_authoritative(self):
        entry = inventory_entry("app/decoders/mca_codes_database.json")
        self.assertEqual(entry["trust"], "INTERNAL_VALIDATED")
        self.assertEqual(entry["score"], 0)
        self.assertTrue(entry["production_eligible"])
